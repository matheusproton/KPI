...
export function DeliveryPlanTableWeekly({ className }: DeliveryPlanTableWeeklyProps) {
  ...
  const [weeklyData, setWeeklyData] = useState<{ [week: number]: WeeklyDeliveryData }>(() => {
    ...
    // Weeks 36-50 for September-December period
    for (let i = 36; i <= 50; i++) {
      data[i] = {
        week: i,
  ...
  const renderEditableWeekCell = (data: WeeklyDeliveryData, week: number) => {
    ...
    return (
      <div
        key={week}
        className={`relative border border-border bg-background flex flex-col items-center justify-center text-xs font-medium transition-all duration-200 aspect-square min-h-[60px] sm:min-h-[70px] md:min-h-[80px] ${
          canClick 
  ...
  // Show weeks 36-42 (7 weeks around current week)
  const visibleWeeks = Array.from({ length: 7 }, (_, i) => 36 + i);
